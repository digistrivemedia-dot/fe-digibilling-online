  ...
  const loadData = async () => {
    try {
      ...
      if (shopData?.quotationValidityDays) {
        const d = new Date();
        d.setDate(d.getDate() + Number(shopData.quotationValidityDays));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = {
        ...
        quotationDate, validityDate, status,
        taxType,
        items: items.map(i => ({
          itemType: i.itemType,
        ...
        // Transport
        ...(shopSettings?.qtEnableTransport && {
        ...
        // PO
        ...(shopSettings?.qtEnablePurchaseOrders && {
          poNumber: po.poNumber,
          poDate: po.poDate || undefined,
        }),
        // Additional
        ...(shopSettings?.qtEnableAdditionalDetails && {
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500">Generate a price quotation for your customer</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-black">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            {items.length > 0 && (
              <div className="hidden md:grid grid-cols-12 gap-2 px-3 mb-2 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                ...
                {shopSettings?.gstScheme !== 'COMPOSITION' && <div className="col-span-1 text-center">GST %</div>}
            ...
            <div className="space-y-3">
              {items.map((item, index) => {
                ...
                const lineTotal = calculateItemWithDiscount(
                  { ...item, gstRate: (shopSettings?.gstScheme === 'COMPOSITION' || taxType === 'NONE') ? 0 : item.gstRate },
                  shopSettings
                ).itemTotal;
                  ...
                  <div key={index}
                    ...
                    {bothEnabled && (
                      <div className="flex items-center gap-2">
                        ...
                        { type: 'service', label: 'Service', Icon: HiLightningBolt, cls: 'purple' }].map(({ type, label, Icon, cls }) => (
                          <button key={type} type="button"
                            ...
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-colors
                              ${item.itemType === type ? `bg-${cls}-600 text-white shadow-sm` : `bg-white text-gray-500 border border-gray-200 hover:bg-${cls}-50`}`}>
          ...
          {/* ── Optional Sections ─────────────────────────────────────── */}
          {(shopSettings?.qtEnableTransport || shopSettings?.qtEnablePurchaseOrders || shopSettings?.qtEnableAdditionalDetails) && (
                ...
                <div>
                  ...
                  {showTransport && (
                    <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {[
                        { label: 'Document Date', key: 'docDate' },
                        { label: 'Supply Date', key: 'supplyDate' },
                      ].map(({ label, key }) => (
                        <div key={key}>
                          <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">{label}</label>
